@echo off
rem scripts\repair.bat
rem Checks program files, bootstrap.json, settings/profiles integrity (restores valid backups) and clears temp files.
rem
...
rem Extra arguments are passed through, e.g.:  repair.bat -Yes
...
"%OCTO_PS%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\octo.ps1" repair %*
