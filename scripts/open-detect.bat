@echo off
rem scripts\open-detect.bat
rem Starts OctoDetect.su (installed copy, or the development build from the repository).
rem
...
rem Extra arguments are passed through, e.g.:  open-detect.bat -Yes
...
"%OCTO_PS%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\octo.ps1" open-detect %*
