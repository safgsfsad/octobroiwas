@echo off
rem scripts\github-update.bat
rem Updates OctoSuite straight from the official GitHub repository.
rem   * installed build   -> newest GitHub release: download, verify (signed manifest or
rem                          SHA256SUMS.txt + Authenticode), back up the configuration, install;
rem   * development copy  -> git pull --ff-only + npm ci (only when the lockfile changed) + npm run build.
rem Options: -CheckOnly (only report), -Yes (no questions), -Lang en|pl.
rem
...
rem Extra arguments are passed through, e.g.:  github-update.bat -CheckOnly
...
"%OCTO_PS%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\octo.ps1" github-update %*
