@echo off
rem scripts\reset-profile.bat
rem Resets one OctoBrowser profile (cookies, site data, history, session). Backup first. Option: -Profile <name|id>.
rem
...
rem Extra arguments are passed through, e.g.:  reset-profile.bat -Yes
...
"%OCTO_PS%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\octo.ps1" reset-profile %*
